...
<script>
	$(function () {
		...
		$('body').on('click','img[id^=img]',function () {
			//去后台查询
			//$(this).attr('id')获取当前元素的id值：'img205'
			var goodsId = $(this).attr('id').substr(3)
			// alert(goodsId)
			window.location.href='goodsDetail.jsp?goodsId='+goodsId
		})
	...
	//处理返回的数据
		function dealWith(currPage){
			//	ajax自动加载商品数据
			$.ajax({
				url:"/goods/goodsListByPageAndCon.do",
				dataType:"json",
				type:"post",
				data:{
					//Y1：传递当前页的条件
					currPage:currPage,
					//Y2：传递条件查询的条件
					goodsNameCon:$("#goodsNameCon").val(),
					priceBeginCon:$("#priceBeginCon").val(),
					priceEndCon:$("#priceEndCon").val(),
					gtIdCon:$("#gtIdCon").val()
                    ...
                    * */
					$.each(data.object.list,function(index,goods){
						var td =
								...
								'<table>' +
								'<tr><td><img src="${pageContext.request.contextPath}/'+goods.imgUrl+'"id="img'+goods.goodsId+'" ></td></tr>' +
	...
	//	处理条件查询
		$("#selectByCon").click(function () {
			dealWith(1)  //这里传参1  因为查询什么条件 都是从第一页开始
		})
